...
export async function POST() {
  try {
    ...
    if (!usage.allowed) return NextResponse.json({ error: "วันนี้คุณรับแสงประจำวันแล้ว กลับมาใหม่พรุ่งนี้นะ" }, { status: 429 });

    const context = await getReflectionContext(user.id);
    const { content, modelUsed } = await generateStructuredReflection({
      task: "daily_light",
      userContext: context,
      prompt: "สร้างแสงประจำวันหนึ่งชุดที่อบอุ่น ไม่ฟันธง และช่วยให้ผู้ใช้สะท้อนใจวันนี้",
    });

    const { data, error } = await supabase
      .from("readings")
      .insert({ user_id: user.id, type: "daily_light", title: "แสงประจำวัน", content, model_used: modelUsed, safety_status: "ok" })
      .select("*")
      .single();
