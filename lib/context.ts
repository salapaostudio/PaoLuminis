...
export async function getReflectionContext(userId: string) {
  const supabase = await createClient();
  const [{ data: profile }, { data: moods }, { data: readings }, { data: journals }, { data: questions }] = await Promise.all([
    ...
    supabase.from("questions").select("category,question_text,created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(5),
  ]);
